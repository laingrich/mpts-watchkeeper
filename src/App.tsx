...
export default function App() {
  ...
  const groups = useMemo(() => {
    const filtered = site.devices.filter(device =>
      `${device.name} ${device.group}`.toLowerCase().includes(query.toLowerCase())
    )
      ...
      <main>
            ...
            {Object.entries(groups).map(([group, devices]) => (
              <div className="device-group" key={group}>
                ...
                <div className="grid cards">
                  {devices.map(device => (
                    <a className="device-card" href={device.url} target="_blank" rel="noreferrer" key={device.name}>
        ...
        {tab === 'Documents' && (
          <section className="panel empty-state">
            ...
            <p>This will connect to the Saltmarsh SharePoint document library.</p>
          </section>
        )}
        ...
        {tab === 'Issues' && (
          <section className="panel empty-state">
            ...
            <p>No open issues. A later API will store issues, updates, owners and service history.</p>
          </section>
        )}
      </main>
    </div>
